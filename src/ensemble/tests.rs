...
mod units {
    ...
    fn generate_concurrent_ensemble_creation() {
        ...
        for handle in handles {
            handle.join().expect("Thread panicked during ensemble generation");
        }
        ...
        assert_eq!(results.len(), 3, "Should receive exactly 3 ensemble results");

        // Verify each result has the correct number of birds and metadata
        let expected_particle_counts = [50, 100, 25];
        let expected_tags = ["concurrent_test_0", "concurrent_test_1", "concurrent_test_2"];
        ...
        for (i, result) in results.iter().enumerate() {
            ...
            assert_eq!(
                ...
                i, expected_particle_counts[i], result.birds.len()
            );

            // Check metadata integrity
            assert_eq!(result.id, i, "Ensemble ID should match");
            assert_eq!(result.tag, expected_tags[i], "Ensemble tag should match");
            assert_eq!(result.params.n_particles, expected_particle_counts[i], "Params should match");

            // Verify all birds are on the sphere surface (within tolerance)
            for (j, bird) in result.birds.iter().enumerate() {
                let distance_from_origin = (bird.position.x.powi(2) + 
                                          bird.position.y.powi(2) + 
                                          bird.position.z.powi(2)).sqrt();
                ...
                assert!(
                    ...
                    j, i, result.params.radius, distance_from_origin
                );
            }
                ...
                for (k, bird2) in result.birds.iter().enumerate() {
                    if j != k {
                        ...
                        assert!(
                            ...
                            j, k, i, distance, result.params.min_distance
                        );
                    }
                }
            }

            println!("✓ Ensemble {}: Generated {} birds with tag '{}' successfully", 
                     i, result.birds.len(), result.tag);
        }
    }
    ...
    fn generate_single_thread_correctness() {
        ...
        assert_eq!(result.params.min_distance, 0.12, "Min distance should match request");

        println!("✓ Single-threaded test: Generated {} birds successfully", result.birds.len());
    }
    ...
    fn generate_stress_test_concurrent_load() {
        ...
        println!("✓ Stress test: {} concurrent ensembles, {} birds each, all successful", 
