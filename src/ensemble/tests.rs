...
mod units {
    ...
    fn validate_on_sphere(birds: &[Bird], radius: f64, tolerance: f64) -> bool {
        birds.iter().all(|bird| {
            (bird.position.norm() - radius).abs() < tolerance
        })
    }
    ...
    fn validate_tangent_velocities(birds: &[Bird], tolerance: f64) -> bool {
        birds.iter().all(|bird| {
            bird.position.dot(&bird.velocity).abs() < tolerance
        })
    }
    ...
    fn validate_speed(birds: &[Bird], expected_speed: f64, tolerance: f64) -> bool {
        birds.iter().all(|bird| {
            (bird.velocity.norm() - expected_speed).abs() < tolerance
        })
    }
    ...
    fn generate_entry_typical() {
        ...
        let files: Vec<_> = fs::read_dir(data_path).unwrap()
            .filter_map(|entry| entry.ok())
            .filter(|entry| entry.file_name().to_string_lossy().contains(&format!("t{}", tag)))
            .collect();
        ...
        assert!(validate_distance_constraints(&entry.birds, params.min_distance, params.radius));
    ...
    fn generate_entry_mpsc_communication() {
        ...
        if let Ok(entries) = fs::read_dir(data_path) {
            for entry in entries.flatten() {
                if entry.file_name().to_string_lossy().contains(&format!("t{}", tag)) {
        ...
        match result {
            Ok(_) => {
                ...
                if file.exists() {
                    ...
                    assert!(validate_distance_constraints(&entry.birds, challenging_params.min_distance, challenging_params.radius));
    ...
    fn concurrent_different_parameters() {
        ...
        let handle1 = thread::spawn(move || {
            generate(tag1, entries_per_tag, params1)
        });
        
        let handle2 = thread::spawn(move || {
            generate(tag2, entries_per_tag, params2)
        });
