...
use crate::ensemble::EntryResult;
use crate::io::{get_current_timestamp, get_data_path, load_data, save_data, DataType};
use std::fs;
use std::path::Path;
use std::sync::mpsc;
use std::thread;
...
pub fn start_receiver_thread(
    ...
    thread::spawn(move || {
        ...
        while let Ok(ensemble_result) = rx.recv() {
            // Add timestamp info
            let ensemble_with_metadata = EntryResult {
...
pub fn load_ensemble(tag: &str, id: &usize) -> Result<EntryResult, Box<dyn std::error::Error>> {
...
pub fn export_to_json(
    ...
    if let Ok(metadata) = std::fs::metadata(output_path) {
        println!(
            "Exported ensemble data to JSON: {} (size: {} bytes)",
            output_path.display(),
            metadata.len()
        );
    }
