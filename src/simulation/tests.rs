...
mod units {
    ...
    fn create_test_simulation(num_birds: usize) -> Simulation {
        ...
        let ensemble_request = EntryGenerationRequest {
            id: 1,
    ...
    fn simulation_run_advances_state() {
        ...
        let initial_positions: Vec<Vec3> =
            sim.current_particles().iter().map(|b| b.position).collect();
        ...
        assert!(
            positions_changed,
            "Positions should change after simulation run"
        );
    }
    ...
    fn simulation_run_executes_correct_iterations() {
        ...
        assert!(
            (sim.current_time() - (max_iterations as f64 * sim.params.dt)).abs() < f64::EPSILON
        );
    }
    ...
    fn simulation_state_buffer_integrity() {
        ...
        assert!(
            state_changed,
            "Simulation state should change after running"
        );
    }
    ...
    fn simulation_deterministic_behavior_with_no_noise() {
        ...
        for (bird1, bird2) in birds1.iter().zip(birds2.iter()) {
            assert!(
                (bird1.position - bird2.position).norm() < 1e-12,
                "Position mismatch: {:?} vs {:?}",
                bird1.position,
                bird2.position
            );
            assert!(
                (bird1.velocity - bird2.velocity).norm() < 1e-12,
